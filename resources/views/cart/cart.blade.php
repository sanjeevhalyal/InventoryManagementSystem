        ...
        foreach ($Cart as $c)
            {
                ...
                $prodcount=\DB::select('select count(*) as c from products WHERE productID=?', [$c->barcode]);

                $photocount=\DB::select('select count(*) as c from product_images WHERE product_id=?', [$c->barcode]);
            ...
            <td data-th="Product">
                <div class="row">
                    ...
                    if($photocount[0]->c>0)
                        {
                            $photo=\DB::select('select cover_image as c from product_images WHERE product_id=?', [$c->barcode]);
                            echo $photo[0]->c;
                        }


                        else{
                        echo "cart/img/noPhoto.png";
        }
                ...
                if($prodcount[0]->c>0)
                {
                    $prod=\DB::select('select name from products WHERE productID=?', [$c->barcode]);
                    echo $prod[0]->name;
                }
