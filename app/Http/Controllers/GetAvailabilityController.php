...
class GetAvailabilityController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        DB::table('cart')->where('extime', '<', time())->delete();
        DB::commit();
...
echo '<div class="panel panel-default">
        ...
        foreach($daterange as $date){
            ...
            $inTran = \DB::select('select count(*) as c from transaction WHERE END_DATE>=? AND START_DATE<=? AND BARCODE_ID=?',
            ...
            if ($inTran[0]->c== 0)
            {
                ...
                $inCart = \DB::select('select count(*) as c from cart WHERE END_DATE>=? AND START_DATE<=? and barcode=?',[$date->format("Y-m-d"),$date->format("Y-m-d"),$request->input("prodid")]);
                if ($inCart[0]->c== 0) {
